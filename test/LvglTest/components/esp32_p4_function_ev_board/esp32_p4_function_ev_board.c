...
#elif CONFIG_BSP_LCD_TYPE_480_800
...
#if CONFIG_BSP_LCD_TYPE_480_800
...
void disp_init(esp_lcd_panel_handle_t* panel_handle)
{
    ESP_LOGI(TAG, "[LCD] Init backlight pin");
    gpio_config_t bk_io_config = {
        .mode = GPIO_MODE_OUTPUT,
        .pin_bit_mask = (uint64_t)1ULL << LCD_BK_LIGHT_PIN,
        .pull_up_en = GPIO_PULLUP_DISABLE,
        .pull_down_en = GPIO_PULLDOWN_DISABLE,
        .intr_type = GPIO_INTR_DISABLE,
    };
    ESP_ERROR_CHECK(gpio_config(&bk_io_config));
    gpio_set_level((gpio_num_t)LCD_BK_LIGHT_PIN, 0);
    ...
    gpio_set_level((gpio_num_t)LCD_BK_LIGHT_PIN, 1);
...
bool touch_driver_read(int16_t* x, int16_t* y)
{
    ...
    if (esp_lcd_touch_get_data(tp_handle, &point_data, &cnt, 1) == ESP_OK && cnt > 0) 
    {
        *x = point_data.x;
        *y = point_data.y;
        return true;
...
#endif // CONFIG_BSP_LCD_TYPE_480_800
