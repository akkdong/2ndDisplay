...
static void msgbox_event_cb(lv_event_t * e)
{
    ...
    if (strcmp(name, "connect") == 0)
    {
        ...
        strcpy(popup->address/*, sizeof(popup->address)*/, lv_textarea_get_text(popup->ta_address));
        popup->port = atoi(lv_textarea_get_text(popup->ta_port));
        strcpy(popup->password/*, sizeof(popup->password)*/, lv_textarea_get_text(popup->ta_password));
...
static void vnc_connect_popup_show(vnc_connect_popup_t* popup)
{
    ...
    */
    ...
    {
        char port[16];
        itoa(popup->port, port/*, sizeof(port)*/, 10);
